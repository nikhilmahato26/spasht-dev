...
export function DataTable<TData extends RowData>({
  ...
  emptyMessage?: string;
  pageSize?: number;
  // Show every row in one internally-scrolling, sticky-header table instead
  // of paginating — matches the Dev Projects table.
  scrollable?: boolean;
  maxHeight?: string;
}) {
  ...
  const [pagination, setPagination] = useState<PaginationState>({
    pageIndex: 0,
    pageSize: scrollable ? Math.max(data.length, 1) : initialPageSize,
  });
  ...
  return (
    <div>
      <div
        className={scrollable ? "overflow-y-auto overflow-x-hidden" : undefined}
        style={scrollable ? { maxHeight } : undefined}
      >
        <Table>
          <TableHeader className={scrollable ? "sticky top-0 z-10 bg-surface-2 [&_tr]:border-b [&_tr]:border-border" : undefined}>
